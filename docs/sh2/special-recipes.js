const SH2_SPECIAL_RECIPES = {
  "Metatron": ["Sandalphon", "Throne", "Mithras"],
  "Shiva": ["Rangda", "Barong"],
  "Huang Long": ["Xuanwu", "Zhuque", "Baihu", "Qing Long"],
  "Black Frost": ["King Frost", "B-Hawaii Frost", "Melon Frost", "Lemon Frost", "Milk-kin Frost", "Strawberry Frost"],
  "Asura": ["Rakshasa", "Hariti", "Dakini"],
  "Sandalphon": ["Power", "Principality", "Archangel", "Angel"],
  "Chernobog": ["Basilisk", "Sandman", "Loa"],
  "Yoshitsune": ["Yatagarasu", "Kurama Tengu", "Koppa Tengu"],
  "Dionysus": ["Mad Gasser", "Turbo Granny"],
  "Melchizedek": ["Angel", "Archangel"],
  "Take-Minakata": ["Koropokkur", "Hare of Inaba", "Onmoraki"],

  "Saki Mitama": ["B-Hawaii Frost x Milk-kin Frost", "Lemon Frost x Strawberry Frost"],
  "Kusi Mitama": ["B-Hawaii Frost x Strawberry Frost", "Melon Frost x Lemon Frost"],
  "Nigi Mitama": ["Melon Frost x Milk-kin Frost", "B-Hawaii Frost x Melon Frost"],
  "Ara Mitama": ["Strawberry Frost x Milk-kin Frost", "Lemon Frost x Milk-kin Frost", "B-Hawaii Frost x Lemon Frost" "Melon Frost x Strawberry Frost"],

  "Ai-ho": ["Jack Frost", "Cironnup"]
}
